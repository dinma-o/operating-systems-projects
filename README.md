...
### 1. Why Per-Partition Mutexes?
**Alternative:** Single global mutex for all partitions

**Choice:** Per-partition mutexes

**Rationale:**
- Allows concurrent writes to different partitions
- Reduces lock contention (P-fold improvement)
- Better scalability with multiple threads
- Minimal overhead (one mutex per partition)

### 2. Why Sorted Key Insertion?
**Alternative:** Unsorted list, sort before Reduce

**Choice:** Maintain sorted order during insertion

**Rationale:**
- Incremental cost during Map phase
- Avoids expensive sort before Reduce
- Enables efficient MR_GetNext iteration
- No additional memory for sorting

### 3. Why Value Tail Pointer?
**Alternative:** Traverse list to append each value

**Choice:** Maintain tail pointer per key

**Rationale:**
- O(1) value append (vs O(n) traversal)
- Significant for keys with many values
- Small memory overhead (one pointer per key)
- Large performance gain in Map phase

### 4. Why qsort for SJF?
**Alternative:** Maintain sorted order during add_job

**Choice:** Sort files/partitions upfront, then add in order

**Rationale:**
- Simpler implementation
- One-time sorting cost
- add_job still inserts in SJF order within queue
- Avoids complex sorting during concurrent job submission
